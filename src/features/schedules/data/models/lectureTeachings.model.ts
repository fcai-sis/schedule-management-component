...
lectureTeachingsSchema.pre("save", async function(next) {
  try {
    ...
    if (!taTeaching) {
      throw new ForeignKeyNotFound("Instructor Teaching not found");
    }

    const section = await mongoose
      .model(lectureModelName)
      .findById(this.lectureId);
    if (!section) {
      throw new ForeignKeyNotFound("Lecture not found");
    }
...
export type lectureTeachingsType = InferSchemaType<
  typeof lectureTeachingsSchema
>;
export const lectureTeachingsModelName = "LectureTeachings";

const LectureTeachingsModel = mongoose.model(
  lectureTeachingsModelName,
  lectureTeachingsSchema
);
